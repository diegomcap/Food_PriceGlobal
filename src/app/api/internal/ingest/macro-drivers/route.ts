import {
  fetchMacroDriversFromFreshSource,
...
async function handleIngest(request: Request) {
  ...
  try {
    const payload = await fetchMacroDriversFromFreshSource();
    const persisted = await persistMacroDrivers({
      items: payload.items,
    ...
    console.error('Macro drivers ingestion job failed:', error);
    return Response.json(
